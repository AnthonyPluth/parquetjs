...
// TODO: pass this through as a parameter
const s3 = new AWS.S3({
...
async function createKMSDecipheriv(kms, ciphertextBlob, iv, encryptionContext) {
  const decryptParams = {
    CiphertextBlob: ciphertextBlob,
    EncryptionContext: encryptionContext,
  }
  return new Promise((resolve, reject) => {
    kms.decrypt(decryptParams, (err, awsDecryptKey) => {
      if (err) {
        reject(err)
      } else {
        // const decipher = createDecipheriv(awsDecryptKey.Plaintext, iv)
        const decipher = crypto.createDecipheriv('AES-256-CBC', awsDecryptKey.Plaintext, iv)
        resolve(decipher)
      }
    })
  }).catch((err) => { throw err })
...
/**
 * decrypts an s3 stream
 *
 * @param {Promise} objectMetadata
 * @returns {Promise<stream>}
 */
 async function createObjectDecipheriv(objectMetadata) {
  const ciphertextBlob = Buffer.from(objectMetadata.Metadata['x-amz-key-v2'], 'base64')
  const objectIV = Buffer.from(objectMetadata.Metadata['x-amz-iv'], 'base64')
  const encryptionContext = JSON.parse(objectMetadata.Metadata['x-amz-matdesc'])

  return createKMSDecipheriv(
    kms,
    ciphertextBlob,
    objectIV,
    encryptionContext,
...
// /**
//  * get decrypted s3 stream
//  *
//  * @param {string} bucket
//  * @param {string} key
//  * @returns {Promise<stream>}
//  */
//  async function getDecryptedS3Stream(bucket, key) {
//   const s3ObjectParams = {
//     Bucket: bucket,
//     Key: key,
//   }

//   const s3ObjectMetadata = await s3.headObject(s3ObjectParams).promise()
//   const decipher = await createObjectDecipheriv(s3ObjectMetadata)
//   decipher.on('error', (e) => {
//     logger.error('decryptedstream has error:', e)
//     throw e
//   })
//   const s3ObjectStream = await s3.getObject(s3ObjectParams).createReadStream()
//   return s3ObjectStream
//     .on('error', (e) => {
//       logger.error('readablestream has error:', e)
//       throw e
//     })
//     .pipe(decipher)
// }
...
class ParquetReader {
   ...
   */
  static async openS3(client, HeadObjectCommand, GetObjectCommand, params, options) {
    let envelopeReader = await ParquetEnvelopeReader.openS3(client, HeadObjectCommand, GetObjectCommand, params, options);
...
class ParquetEnvelopeReader {
  ...
  static async openS3(client, HeadObjectCommand, GetObjectCommand, params, options) {
    const s3ObjectMetadata = await s3.headObject(params).promise()
    
    let fileStat = async () => {
      try {
        // let headObjectCommand = await s3.send(new HeadObjectCommand (params));
        // console.log(`sent headObjectCommand. Response: ${headObjectCommand}`)
        ...
        return Promise.reject(`rejected headObjectCommand: ${err.message}`);
      }
    }

    // Decrypt stream
    // const s3ObjectMetadata = await s3.headObject(params).promise()
    // console.log('object metadata', s3ObjectMetadata)
    ...
    let readFn = async (offset, length, file) => {
      ...
      try {
        // let getObjectCommand = await s3.send(
        //   new GetObjectCommand({Range, ...params})
        // );
        // console.log("sent GetObjectCommand")
        const s3ObjectStream = await s3.getObject(params).createReadStream()
        const stream = s3ObjectStream
          .on('error', (e) => {
            logger.error('readablestream has error:', e)
            throw e
          })
      ...
      //         } else {
      //           bodyBuffer = stream.read() || bodyBuffer;
      //         }
      ...
      // return Promise.resolve(body);
        ...
        console.log(`readfn Error: ${err}`)
        return Promise.reject(`rejected getObject command: ${err.message}`);
      }
    };
    console.log('got readFn', readFn)
